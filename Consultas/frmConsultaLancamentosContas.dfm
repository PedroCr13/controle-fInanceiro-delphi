inherited frm_ConsultaLancamentosContas: Tfrm_ConsultaLancamentosContas
  Left = 488
  Top = 212
  Caption = 'Lan'#231'amentos - Hist'#243'rico Movimenta'#231#227'o Conta'
  ClientWidth = 573
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Top = 439
    Width = 573
    Height = 34
    object btnAdicionar: TSpeedButton
      Left = 4
      Top = 3
      Width = 95
      Height = 29
      Action = actAdicionar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object btnEditar: TSpeedButton
      Left = 99
      Top = 3
      Width = 95
      Height = 29
      Action = actEditar
      Glyph.Data = {
        
        39009C7B6F006D789F007794FF00637BDE00CECEC600FF00FF00}
    end
  end
  inherited Panel2: TPanel
    Width = 573
    Height = 374
    inherited DBGrid1: TDBGrid
      Width = 571
      Height = 372
      OnDblClick = actEditarExecute
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'LCTO'
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o do Lanc'#231'amento'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 460
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'MOVIMENTO'
          Title.Alignment = taCenter
          Title.Caption = 'Movimento'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 78
          Visible = True
        end>
    end
  end
  inherited Panel3: TPanel
    Width = 573
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 571
      Height = 63
      Align = alClient
      Caption = 'Localizar'
      TabOrder = 0
      object btnLimpar: TSpeedButton
        Left = 467
        Top = 18
        Width = 95
        Height = 29
        Action = actLimpar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object btnConfirmar: TSpeedButton
        Left = 373
        Top = 18
        Width = 94
        Height = 29
        Action = actPesquisar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object Label1: TLabel
        Left = 128
        Top = 27
        Width = 58
        Height = 13
        Caption = 'Descri'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object cbbFiltro: TComboBox
        Left = 18
        Top = 22
        Width = 87
        Height = 21
        Style = csDropDownList
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 0
        Text = 'Exibir Todos'
        Items.Strings = (
          'Exibir Todos'
          'Credito'
          'Debito')
      end
      object edtPesquisa: TEdit
        Left = 195
        Top = 22
        Width = 171
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 1
      end
    end
  end
  inherited ActionList1: TActionList
    Images = ImageList1
    Left = 240
    Top = 128
    object actAdicionar: TAction
      Caption = 'Adicionar'
      ImageIndex = 27
      OnExecute = actAdicionarExecute
    end
    object actEditar: TAction
      Caption = 'Editar'
      ImageIndex = 17
      OnExecute = actEditarExecute
    end
    object actPesquisar: TAction
      Caption = 'Pesquisar'
      ImageIndex = 22
      OnExecute = actPesquisarExecute
    end
    object actLimpar: TAction
      Caption = 'Limpar'
      ImageIndex = 1
      OnExecute = actLimparExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 240
    Top = 88
  end
  inherited qryConsulta: TSQLQuery
    Params = <
      item
        DataType = ftInteger
        Name = 'USUARIO'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT L.CODIGO,'
      '       L.LCTO,'
      '       L.MOVIMENTO,'
      '       L.COD_USUARIO'
      'FROM TB_LANCAMENTOS L'
      'WHERE L.COD_USUARIO =:USUARIO'
      'ORDER BY L.LCTO')
    object qryConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object qryConsultaLCTO: TStringField
      FieldName = 'LCTO'
      Size = 100
    end
    object qryConsultaMOVIMENTO: TStringField
      FieldName = 'MOVIMENTO'
      FixedChar = True
      Size = 1
    end
    object qryConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
  inherited cdsConsulta: TClientDataSet
    object cdsConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cdsConsultaLCTO: TStringField
      FieldName = 'LCTO'
      Size = 100
    end
    object cdsConsultaMOVIMENTO: TStringField
      FieldName = 'MOVIMENTO'
      FixedChar = True
      Size = 1
    end
    object cdsConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
end
