inherited frm_ConsultaTiposAplicacoes: Tfrm_ConsultaTiposAplicacoes
  Caption = 'Esp'#233'cies de Aplica'#231#245'es'
  ClientWidth = 610
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Top = 439
    Width = 610
    Height = 34
    object btnAdicionar: TSpeedButton
      Left = 4
      Top = 3
      Width = 95
      Height = 29
      Action = actAdicionar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object btnEditar: TSpeedButton
      Left = 99
      Top = 3
      Width = 95
      Height = 29
      Action = actEditar
      Glyph.Data = {
        
        39009C7B6F006D789F007794FF00637BDE00CECEC600FF00FF00}
    end
  end
  inherited Panel2: TPanel
    Width = 610
    Height = 374
    inherited DBGrid1: TDBGrid
      Width = 608
      Height = 372
      OnDblClick = actEditarExecute
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Title.Alignment = taCenter
          Title.Caption = 'Aplica'#231#227'o'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 308
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CATEGORIA'
          Title.Caption = 'Categoria'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'DESC_CATEGORIA'
          Title.Alignment = taCenter
          Title.Caption = 'Categoria'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 234
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'IMPOSTO_RENDA'
          Title.Alignment = taCenter
          Title.Caption = 'I.R.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 35
          Visible = True
        end>
    end
  end
  inherited Panel3: TPanel
    Width = 610
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 608
      Height = 63
      Align = alClient
      Caption = 'Localizar'
      TabOrder = 0
      object btnLimpar: TSpeedButton
        Left = 494
        Top = 18
        Width = 95
        Height = 29
        Action = actLimpar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object btnConfirmar: TSpeedButton
        Left = 399
        Top = 18
        Width = 95
        Height = 29
        Action = actPesquisar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object Label1: TLabel
        Left = 145
        Top = 27
        Width = 58
        Height = 13
        Caption = 'Descri'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edtPesquisa: TEdit
        Left = 212
        Top = 23
        Width = 178
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
      end
      object cbbFiltro: TComboBox
        Left = 16
        Top = 23
        Width = 118
        Height = 21
        Style = csDropDownList
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 1
        Text = 'Aplica'#231#227'o'
        Items.Strings = (
          'Aplica'#231#227'o'
          'Categoria')
      end
    end
  end
  inherited ActionList1: TActionList
    Images = ImageList1
    Left = 344
    Top = 152
    object actAdicionar: TAction
      Caption = 'Adicionar'
      ImageIndex = 27
      OnExecute = actAdicionarExecute
    end
    object actEditar: TAction
      Caption = 'Editar'
      ImageIndex = 17
      OnExecute = actEditarExecute
    end
    object actPesquisar: TAction
      Caption = 'Pesquisar'
      ImageIndex = 22
      OnExecute = actPesquisarExecute
    end
    object actLimpar: TAction
      Caption = 'Limpar'
      ImageIndex = 1
      OnExecute = actLimparExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 344
    Top = 112
  end
  inherited qryConsulta: TSQLQuery
    Params = <
      item
        DataType = ftInteger
        Name = 'USUARIO'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT T.CODIGO,'
      '       T.DESCRICAO,'
      '       T.CATEGORIA,'
      '       C.DESCRICAO AS "DESC_CATEGORIA",'
      '       T.IMPOSTO_RENDA,'
      '       T.COD_USUARIO'
      'FROM TB_TIPOAPLICACOES T'
      'INNER JOIN TB_CATEGORIA_APLICACAO C ON (C.CODIGO = T.CATEGORIA)'
      'WHERE T.CODIGO > 0'
      'AND T.COD_USUARIO =:USUARIO'
      'ORDER BY T.DESCRICAO')
    object qryConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object qryConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 100
    end
    object qryConsultaCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
    end
    object qryConsultaDESC_CATEGORIA: TStringField
      FieldName = 'DESC_CATEGORIA'
      Size = 100
    end
    object qryConsultaIMPOSTO_RENDA: TStringField
      FieldName = 'IMPOSTO_RENDA'
      FixedChar = True
      Size = 1
    end
  end
  inherited cdsConsulta: TClientDataSet
    object cdsConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cdsConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 100
    end
    object cdsConsultaCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
    end
    object cdsConsultaDESC_CATEGORIA: TStringField
      FieldName = 'DESC_CATEGORIA'
      Size = 100
    end
    object cdsConsultaIMPOSTO_RENDA: TStringField
      FieldName = 'IMPOSTO_RENDA'
      FixedChar = True
      Size = 1
    end
  end
end
