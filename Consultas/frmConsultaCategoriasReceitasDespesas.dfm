inherited frm_ConsultaCategoriasReceitasDespesas: Tfrm_ConsultaCategoriasReceitasDespesas
  Caption = 'Categorias Receitas e Despesas'
  ClientWidth = 617
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Top = 439
    Width = 617
    Height = 34
    object btnAdicionar: TSpeedButton
      Left = 4
      Top = 3
      Width = 95
      Height = 29
      Action = actAdicionar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object btnEditar: TSpeedButton
      Left = 99
      Top = 3
      Width = 95
      Height = 29
      Action = actEditar
      Glyph.Data = {
        
        39009C7B6F006D789F007794FF00637BDE00CECEC600FF00FF00}
    end
  end
  inherited Panel2: TPanel
    Width = 617
    Height = 374
    inherited DBGrid1: TDBGrid
      Width = 615
      Height = 372
      OnDblClick = actEditarExecute
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'CATEGORIA'
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o da Categoria'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 419
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'genero'
          Title.Alignment = taCenter
          Title.Caption = 'G'#234'nero'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 163
          Visible = True
        end>
    end
  end
  inherited Panel3: TPanel
    Width = 617
    object GroupBox2: TGroupBox
      Left = 1
      Top = 1
      Width = 615
      Height = 63
      Align = alClient
      Caption = 'Localizar'
      TabOrder = 0
      object btnLimpar: TSpeedButton
        Left = 494
        Top = 18
        Width = 95
        Height = 29
        Action = actLimpar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object btnConfirmar: TSpeedButton
        Left = 399
        Top = 18
        Width = 95
        Height = 29
        Action = actPesquisa
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object Label1: TLabel
        Left = 145
        Top = 27
        Width = 58
        Height = 13
        Caption = 'Descri'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edtPesquisa: TEdit
        Left = 212
        Top = 23
        Width = 178
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
      end
      object cbbFiltro: TComboBox
        Left = 16
        Top = 23
        Width = 118
        Height = 21
        Style = csDropDownList
        ItemHeight = 13
        ItemIndex = 0
        TabOrder = 1
        Text = 'Exibir Todos'
        Items.Strings = (
          'Exibir Todos'
          'Despesas'
          'Receitas'
          'Investimentos')
      end
    end
  end
  inherited ActionList1: TActionList
    Images = ImageList1
    Left = 456
    Top = 152
    object actAdicionar: TAction
      Caption = 'Adicionar'
      ImageIndex = 27
      OnExecute = actAdicionarExecute
    end
    object actEditar: TAction
      Caption = 'Editar'
      ImageIndex = 17
      OnExecute = actEditarExecute
    end
    object actPesquisa: TAction
      Caption = 'Pesquisa'
      ImageIndex = 22
      OnExecute = actPesquisaExecute
    end
    object actLimpar: TAction
      Caption = 'Limpar'
      ImageIndex = 1
      OnExecute = actLimparExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 456
    Top = 112
  end
  inherited qryConsulta: TSQLQuery
    Params = <
      item
        DataType = ftInteger
        Name = 'USUARIO'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT C.CODIGO,'
      '       C.CATEGORIA,'
      '       C.CREDITO_DEBITO,'
      '       C. COD_USUARIO'
      'FROM TB_CATEGORIA C'
      'WHERE C.CODIGO > 0'
      'AND C.COD_USUARIO =:USUARIO'
      'ORDER BY C.CATEGORIA')
    object qryConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object qryConsultaCATEGORIA: TStringField
      FieldName = 'CATEGORIA'
      Size = 100
    end
    object qryConsultaCREDITO_DEBITO: TStringField
      FieldName = 'CREDITO_DEBITO'
      FixedChar = True
      Size = 1
    end
    object qryConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
  inherited cdsConsulta: TClientDataSet
    OnCalcFields = cdsConsultaCalcFields
    object cdsConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cdsConsultaCATEGORIA: TStringField
      FieldName = 'CATEGORIA'
      Size = 100
    end
    object cdsConsultaCREDITO_DEBITO: TStringField
      FieldName = 'CREDITO_DEBITO'
      FixedChar = True
      Size = 1
    end
    object cdsConsultagenero: TStringField
      FieldKind = fkCalculated
      FieldName = 'genero'
      Calculated = True
    end
    object cdsConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
end
