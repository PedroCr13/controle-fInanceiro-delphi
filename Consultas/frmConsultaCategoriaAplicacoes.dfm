inherited frm_ConsultaCategoriaAplicacoes: Tfrm_ConsultaCategoriaAplicacoes
  Caption = 'Categorias de Aplica'#231#245'es'
  ClientHeight = 448
  ClientWidth = 530
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Top = 414
    Width = 530
    Height = 34
    object btnAdicionar: TSpeedButton
      Left = 4
      Top = 3
      Width = 95
      Height = 29
      Action = actAdicionar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object btnEditar: TSpeedButton
      Left = 99
      Top = 3
      Width = 95
      Height = 29
      Action = actEditar
      Glyph.Data = {
        
        39009C7B6F006D789F007794FF00637BDE00CECEC600FF00FF00}
    end
  end
  inherited Panel2: TPanel
    Width = 530
    Height = 349
    inherited DBGrid1: TDBGrid
      Width = 528
      Height = 347
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o da Categoria e Aplica'#231#227'o'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 502
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'COD_USUARIO'
          Visible = False
        end>
    end
  end
  inherited Panel3: TPanel
    Width = 530
    object grp1: TGroupBox
      Left = 1
      Top = 1
      Width = 528
      Height = 63
      Align = alClient
      Caption = 'Localizar'
      TabOrder = 0
      object btnLimpar: TSpeedButton
        Left = 411
        Top = 18
        Width = 95
        Height = 29
        Action = actLimpar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object btnConfirmar: TSpeedButton
        Left = 316
        Top = 18
        Width = 95
        Height = 29
        Action = actPesquisar
        Glyph.Data = {
          
          FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      end
      object lbl2: TLabel
        Left = 18
        Top = 26
        Width = 58
        Height = 13
        Caption = 'Descri'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object edtPesquisa: TEdit
        Left = 84
        Top = 22
        Width = 225
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
      end
    end
  end
  inherited ActionList1: TActionList
    Images = ImageList1
    Left = 120
    Top = 184
    object actAdicionar: TAction
      Caption = 'Adicionar'
      ImageIndex = 27
      OnExecute = actAdicionarExecute
    end
    object actEditar: TAction
      Caption = 'Editar'
      ImageIndex = 17
      OnExecute = actEditarExecute
    end
    object actPesquisar: TAction
      Caption = 'Pesquisar'
      ImageIndex = 22
      OnExecute = actPesquisarExecute
    end
    object actLimpar: TAction
      Caption = 'Limpar'
      ImageIndex = 1
      OnExecute = actLimparExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 120
    Top = 144
  end
  inherited qryConsulta: TSQLQuery
    Params = <
      item
        DataType = ftInteger
        Name = 'USUARIO'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'SELECT C.CODIGO,'
      '       C.DESCRICAO,'
      '       C.COD_USUARIO'
      'FROM TB_CATEGORIA_APLICACAO C'
      'WHERE C.COD_USUARIO =:USUARIO'
      'AND C.CODIGO > 0'
      'ORDER BY C.DESCRICAO')
    Left = 72
    Top = 136
    object qryConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object qryConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 100
    end
    object qryConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
  inherited dspConsulta: TDataSetProvider
    Left = 72
    Top = 176
  end
  inherited cdsConsulta: TClientDataSet
    Left = 72
    Top = 216
    object cdsConsultaCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object cdsConsultaDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 100
    end
    object cdsConsultaCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
  end
  inherited dsConsulta: TDataSource
    Left = 72
    Top = 256
  end
end
